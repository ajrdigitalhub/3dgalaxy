...
export const routes: Routes = [
  ...
  { path: 'slicer', loadComponent: () => import('./pages/printing-service/printing-service').then(m => m.PrintingService) },
  { 
    path: 'cart', 
    loadComponent: () => import('./pages/cart/cart').then(m => m.CartCheckout),
    data: { preload: true }
  },
  { 
    path: 'checkout', 
  ...
  { path: 'recover-cart/:token', loadComponent: () => import('./pages/recover-cart/recover-cart').then(m => m.RecoverCartComponent) },
  { path: 'order-success', loadComponent: () => import('./pages/checkout/order-success').then(m => m.OrderSuccessComponent) },
  { path: 'order-tracking', loadComponent: () => import('./pages/order-tracking/order-tracking').then(m => m.OrderTrackingComponent) },
  { 
    path: 'orders', 
    loadComponent: () => import('./pages/orders/orders').then(m => m.OrdersTracking), 
  ...
  { 
    path: 'admin', 
    loadComponent: () => import('./pages/admin/admin').then(m => m.AdminPanel), 
    canActivate: [roleGuard], 
    data: { roles: ['Admin', 'Manager', 'Super Admin', 'admin', 'super-admin'] } 
  },
