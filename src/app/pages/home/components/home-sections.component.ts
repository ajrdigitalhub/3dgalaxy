...
@Component({
  ...
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6" appScrollReveal="fade">
      <div class="bg-white dark:bg-neutral-900 border border-neutral-200/60 dark:border-neutral-800/60 rounded-[3rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-8 md:p-12 lg:py-16">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center">
          <div class="col-span-1 md:col-span-7 space-y-6 text-left order-2 md:order-1" appScrollReveal="slide-up">
            <div class="space-y-1">
              <span class="text-[10px] font-extrabold uppercase tracking-[0.25em] text-neutral-400 select-text">{{ section.config['brand'] || 'CREALITY' }}</span>
              <h2 class="text-4xl md:text-5xl font-black text-neutral-900 dark:text-white tracking-tighter leading-none font-display select-text">
                {{ section.config['name'] || 'Creality Sparx i7 Combo' }}
              </h2>
            </div>
            <p class="text-neutral-500 dark:text-neutral-400 text-sm leading-relaxed font-normal max-w-xl">
              {{ section.config['description'] || 'Professional grade dual extrusion setup engineered for high efficiency, high speed fabrication runs.' }}
            </p>
            <div class="flex flex-wrap items-center gap-4">
              <span class="text-2xl font-black text-neutral-900 dark:text-white">{{ salePrice() | currency:'INR':'symbol':'1.0-0' }}</span>
              <span class="text-sm text-neutral-400 line-through font-medium">{{ mrp() | currency:'INR':'symbol':'1.0-0' }}</span>
            ...
            <div class="pt-2">
              <button (click)="addToCartById('prod-6')"
                class="h-12 px-8 bg-[#f54f00] hover:bg-orange-700 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:scale-[1.03] transition-all shadow-md shadow-orange-500/10 flex items-center gap-2 cursor-pointer">
                <span>BUY NOW</span>
                <mat-icon class="scale-95">shopping_cart</mat-icon>
              </button>
          ...
          <div class="col-span-1 md:col-span-5 flex justify-center order-1 md:order-2" appScrollReveal="scale-in">
            <img [src]="section.config['image'] || 'https://store.bambulab.com/cdn/shop/files/X1C_Combo_800x800.png'"
...
export class HomeShowcaseTwoComponent {
  ...
  salePrice = computed(() => asNumber(this._section()?.config?.['salePrice'] || 75000));
...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 py-12 flex flex-col items-center justify-center text-center space-y-6" appScrollReveal="scale-in">
      <div class="space-y-2">
        <h1 class="text-5xl md:text-6xl font-black tracking-widest text-neutral-950 dark:text-white uppercase font-display select-text leading-none">
          {{ section.config['title'] || 'PREMIUM FILAMENT' }}</h1>
        <h2 class="text-[10px] font-black uppercase tracking-[0.3em] text-[#d65108]">{{ section.config['subtitle'] || 'Ecosystem Materials' }}</h2>
      </div>
      <div class="pt-4">
        <button (click)="selectFilterCategory('materials')"
...
@Component({
  ...
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 py-12 flex flex-col items-center justify-center text-center space-y-6" appScrollReveal="scale-in">
      <div class="space-y-2">
        <h1 class="text-5xl md:text-6xl font-black tracking-widest text-neutral-950 dark:text-white uppercase font-display select-text leading-none">
          {{ section.config['title'] || 'SPARE PARTS' }}</h1>
        <h2 class="text-[10px] font-black uppercase tracking-[0.3em] text-[#d65108]">{{ section.config['subtitle'] || 'Original Components' }}</h2>
      </div>
      <div class="pt-4">
        <button (click)="selectFilterCategory('spare-parts')"
          class="h-11 px-8 bg-neutral-950 hover:bg-neutral-800 dark:bg-white dark:hover:bg-neutral-100 text-white dark:text-neutral-950 rounded-lg font-black text-xs uppercase tracking-widest transition-all shadow-md cursor-pointer">
          {{ section.config['buttonText'] || 'VIEW ALL' }}
        </button>
      </div>
...
@Component({
  ...
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 py-12 flex flex-col items-center justify-center text-center space-y-6" appScrollReveal="scale-in">
      <div class="space-y-2">
        <h1 class="text-5xl md:text-6xl font-black tracking-widest text-neutral-950 dark:text-white uppercase font-display select-text leading-none">
          {{ section.config['title'] || '3D PRINTERS' }}</h1>
        <h2 class="text-[10px] font-black uppercase tracking-[0.3em] text-[#d65108]">{{ section.config['subtitle'] || 'Flagship Systems' }}</h2>
      </div>
      <div class="pt-4">
        <button (click)="selectFilterCategory('3d-printers')"
          class="h-11 px-8 bg-neutral-950 hover:bg-neutral-800 dark:bg-white dark:hover:bg-neutral-100 text-white dark:text-neutral-950 rounded-lg font-black text-xs uppercase tracking-widest transition-all shadow-md cursor-pointer">
          {{ section.config['buttonText'] || 'VIEW ALL' }}
        </button>
      </div>
...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 space-y-12" appScrollReveal="fade">
      <div class="text-center space-y-2">
        <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-orange-600 font-display">
          {{ section.config['subtitle'] || 'Browse Collections' }}</h2>
        <h3 class="text-3xl font-black text-neutral-900 dark:text-white tracking-tighter font-display">
          {{ section.config['title'] || 'Shop By Category' }}</h3>
      </div>

      <div class="space-y-12">
        @for (group of shopByCategoryGroups(); track group.category.id; let idx = $index) {
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            <!-- Large Card -->
            <div class="lg:col-span-5 bg-neutral-100 dark:bg-neutral-900 rounded-[2.5rem] overflow-hidden relative group p-8 md:p-12 flex flex-col justify-end min-h-[500px] lg:min-h-[550px]"
              [class.lg:order-2]="idx % 2 !== 0" appScrollReveal="scale-in">
              <img [src]="group.category.image || 'https://images.unsplash.com/photo-1631035626723-cd8e9ef9e728?auto=format&fit=crop&q=80&w=800'"
                class="absolute inset-0 w-full h-full object-cover transition-transform duration-[2000ms] ease-out group-hover:scale-105"
                referrerpolicy="no-referrer" loading="lazy" decoding="async">
              <div class="absolute inset-0 bg-gradient-to-t from-neutral-950/95 via-neutral-900/60 to-transparent pointer-events-none"></div>

              <div class="relative z-10 space-y-4 text-left">
                <h4 class="text-4xl font-black text-white tracking-tight uppercase font-display">{{ group.category.name }}</h4>
                <p class="text-neutral-300 text-sm leading-relaxed max-w-sm line-clamp-2">{{ group.category.description || 'Explore the latest collection of premium ' + group.category.name + ' for your next project.' }}</p>
                <a [routerLink]="['/category', group.category.slug]"
                  ...
                  VIEW COLLECTION <mat-icon class="scale-75">arrow_forward</mat-icon>
                </a>
              </div>
            </div>

            <!-- Medium Cards Grid -->
            <div class="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6" [class.lg:order-1]="idx % 2 !== 0" appScrollReveal="fade">
              @for (p of group.products; track p.id; let subIdx = $index) {
                <a [routerLink]="['/product', p.slug]" appScrollReveal="slide-up" [delay]="subIdx * 100" appTilt [tiltMax]="5"
                  ...
                  <div class="w-32 h-32 md:w-36 md:h-36 bg-neutral-50 dark:bg-neutral-950/40 rounded-2xl flex items-center justify-center p-1.5 shrink-0 overflow-hidden border border-neutral-100 dark:border-neutral-800/30">
                    <img [src]="p.images && p.images[0]?.url || p.images && p.images[0] || 'https://via.placeholder.com/150'"
                      class="w-full h-full object-contain transform group-hover/item:scale-110 transition-transform duration-700 ease-out"
                      referrerpolicy="no-referrer" loading="lazy" decoding="async">
                  </div>
                  <div class="flex-1 min-w-0 flex flex-col justify-center space-y-1.5 text-left">
                    <span class="text-[9px] font-extrabold uppercase tracking-widest text-[#d65108] truncate">{{ p.brand }}</span>
                    <h5 class="text-sm font-bold text-neutral-900 dark:text-neutral-100 line-clamp-2 leading-snug group-hover/item:text-[#d65108] transition-colors">
                      {{ p.name }}</h5>
                    <div class="pt-2 flex items-center justify-between">
                      <span class="text-sm font-black dark:text-white">{{ p.activePrice | currency:'INR':'symbol':'1.0-0' }}</span>
                      <div class="h-8 w-8 bg-neutral-100 dark:bg-neutral-800 rounded-full flex items-center justify-center text-neutral-400 group-hover/item:bg-[#d65108] group-hover/item:text-white transition-colors duration-300">
    ...
    for (const slug of targetSlugs) {
      ...
      if (category) {
        ...
        const mappedProducts = catProducts.map(p => ({
          ...p,
          activePrice: isDealer ? (p.dealerPrice || p.dealer_price || p.salePrice || p.sale_price) : (p.salePrice || p.sale_price)
        }));
        groups.push({ category, products: mappedProducts });
      }
    }
    return groups;
  });
  ...
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 space-y-12" appScrollReveal="fade">
      <div class="text-center space-y-2 max-w-2xl mx-auto">
        <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-[#d65108] font-display">
          {{ section.config['subtitle'] || 'The Laboratory' }}</h2>
        <h3 class="text-3xl font-black text-neutral-900 dark:text-white tracking-tighter leading-tight font-display">
          {{ section.config['title'] || 'Shop by Technology' }}</h3>
        <p class="text-neutral-500 dark:text-neutral-400 text-sm">Expertly matched systems for diverse rapid physical manufacturing criteria.</p>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        @for (tech of technologies; track tech.id; let idx = $index) {
          <div appScrollReveal="slide-up" [delay]="idx * 100" appTilt [tiltMax]="5"
            ...
            <div class="absolute inset-0 p-8 flex flex-col justify-between z-10 text-left">
              <div class="h-12 w-12 bg-white/25 backdrop-blur-md border border-white/20 rounded-xl flex items-center justify-center text-white">
                <mat-icon class="scale-110">{{ tech.icon }}</mat-icon>
              </div>
              <div class="space-y-3">
                <h4 class="text-2xl font-black text-white tracking-tight">{{ tech.name }}</h4>
                <p class="text-neutral-300 text-xs font-normal leading-relaxed">{{ tech.desc }}</p>
                <a [routerLink]="['/category', tech.id === 'industrial' ? '3d-printers' : tech.id]"
...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section [id]="section.id" appScrollReveal="fade"
      class="relative py-24 bg-neutral-900 rounded-[3rem] border border-neutral-800/40 overflow-hidden mx-6 text-left">
      <div class="absolute top-0 right-0 w-[800px] h-[800px] bg-orange-500/5 blur-[150px] rounded-full translate-x-1/2 -translate-y-1/2"></div>
      <div class="relative z-10 max-w-7xl mx-auto px-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div class="lg:col-span-4 space-y-8 col-span-1" appScrollReveal="slide-up">
          <div class="space-y-3">
            <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-orange-500 font-display">
              {{ section.config['subtitle'] || 'Enterprise Protocol' }}</h2>
            <h3 class="text-5xl font-black text-white tracking-tighter leading-tight font-display">
              {{ section.config['title'] || 'Industrial Solutions' }}</h3>
            <p class="text-neutral-400 text-sm leading-relaxed">Unified 3D setups, industrial consulting, dental molds, and robotic kits formulated for scaling companies.</p>
          </div>
          <a [routerLink]="['/printing-service']"
            ...
            BOOK CONSULTATION <mat-icon class="scale-90">support_agent</mat-icon>
          </a>
        </div>
        <div class="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6" appScrollReveal="fade">
          @for (s of solutions; track s.name; let idx = $index) {
            <div appScrollReveal="slide-up" [delay]="idx * 100" appTilt [tiltMax]="5"
              ...
              <div class="h-12 w-12 bg-orange-500/10 rounded-xl flex items-center justify-center text-orange-500 mb-6 transition-transform group-hover:scale-110">
                <mat-icon class="scale-110">{{ s.icon }}</mat-icon>
              </div>
              <h4 class="text-xl font-bold text-white mb-2">{{ s.name }}</h4>
              <p class="text-neutral-500 text-xs font-normal leading-relaxed mb-4">{{ s.desc }}</p>
              <a routerLink="/printing-service"
                class="text-[10px] font-black uppercase tracking-widest text-[#d65108] hover:text-orange-400 flex items-center gap-2">
                LEARN MORE <mat-icon class="text-sm scale-75">arrow_forward</mat-icon>
              </a>
            </div>
          }
        </div>
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, MatIconModule, TiltDirective, ScrollRevealDirective],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 space-y-12 py-12" appScrollReveal="fade">


      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Service 1 -->
        <div appTilt [tiltMax]="6" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2rem] flex flex-col justify-between h-[320px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] hover:border-orange-500/30 transition-all duration-300 group">
          <div class="h-12 w-12 bg-orange-500/10 rounded-xl flex items-center justify-center text-[#d65108] group-hover:scale-110 transition-transform">
            <mat-icon class="scale-110">layers</mat-icon>
          </div>
          <div class="space-y-2 text-left">
            <h4 class="text-lg font-black text-neutral-900 dark:text-white leading-tight">Instant 3D Print Quoting</h4>
            <p class="text-xs text-neutral-500 leading-relaxed">Upload any STL or STEP file and receive dynamic, tailored slicing quotes with infill controls in seconds.</p>
          </div>
          <a routerLink="/printing-service" class="text-[10px] font-black uppercase tracking-widest text-[#d65108] hover:text-orange-400 flex items-center gap-2 mt-4 text-left">
            UPLOAD CAD FILE <mat-icon class="text-sm scale-75">arrow_forward</mat-icon>
          </a>
        </div>

        <!-- Service 2 -->
        <div appTilt [tiltMax]="6" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2rem] flex flex-col justify-between h-[320px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] hover:border-orange-500/30 transition-all duration-300 group">
          <div class="h-12 w-12 bg-purple-500/10 rounded-xl flex items-center justify-center text-purple-500 group-hover:scale-110 transition-transform">
            <mat-icon class="scale-110">school</mat-icon>
          </div>
          <div class="space-y-2 text-left">
            <h4 class="text-lg font-black text-neutral-900 dark:text-white leading-tight">Academy & Maintenance</h4>
            <p class="text-xs text-neutral-500 leading-relaxed">Certified training modules for industrial print mechanics, parameter profiles, and slicing calibrations.</p>
          </div>
          <a routerLink="/printing-service" class="text-[10px] font-black uppercase tracking-widest text-[#d65108] hover:text-orange-400 flex items-center gap-2 mt-4 text-left">
            START LEARNING <mat-icon class="text-sm scale-75">arrow_forward</mat-icon>
          </a>
        </div>

        <!-- Service 3 -->
        <div appTilt [tiltMax]="6" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2rem] flex flex-col justify-between h-[320px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] hover:border-orange-500/30 transition-all duration-300 group">
          <div class="h-12 w-12 bg-blue-500/10 rounded-xl flex items-center justify-center text-blue-500 group-hover:scale-110 transition-transform">
            <mat-icon class="scale-110">build_circle</mat-icon>
          </div>
          <div class="space-y-2 text-left">
            <h4 class="text-lg font-black text-neutral-900 dark:text-white leading-tight">Annual Maintenance (AMC)</h4>
            <p class="text-xs text-neutral-500 leading-relaxed">Preventive care contracts including physical parts replacements, firmware audits, and calibration sweeps.</p>
          </div>
          <a routerLink="/printing-service" class="text-[10px] font-black uppercase tracking-widest text-[#d65108] hover:text-orange-400 flex items-center gap-2 mt-4 text-left">
            REQUEST AMC QUOTE <mat-icon class="text-sm scale-75">arrow_forward</mat-icon>
          </a>
        </div>

        <!-- Service 4 -->
        <div appTilt [tiltMax]="6" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2rem] flex flex-col justify-between h-[320px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.04)] hover:border-orange-500/30 transition-all duration-300 group">
          <div class="h-12 w-12 bg-emerald-500/10 rounded-xl flex items-center justify-center text-emerald-500 group-hover:scale-110 transition-transform">
            <mat-icon class="scale-110">hub</mat-icon>
          </div>
          <div class="space-y-2 text-left">
            <h4 class="text-lg font-black text-neutral-900 dark:text-white leading-tight">Farm System Integration</h4>
            <p class="text-xs text-neutral-500 leading-relaxed">Establish automated production pipelines using centralized dashboard panels and robotic filament loading structures.</p>
          </div>
          <a routerLink="/printing-service" class="text-[10px] font-black uppercase tracking-widest text-[#d65108] hover:text-orange-400 flex items-center gap-2 mt-4 text-left">
            TALK TO ENGINEER <mat-icon class="text-sm scale-75">arrow_forward</mat-icon>
          </a>
        </div>
      </div>
    </section>
  ...
  template: `
    <section [id]="section.id" class="max-w-5xl mx-auto px-6 py-20">
      <div class="text-center space-y-2 mb-12" appScrollReveal="fade">
        <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-orange-600 font-display">THE GALAXY PROTOCOL</h2>
        <h3 class="text-4xl font-black text-neutral-900 dark:text-white tracking-tighter font-display">Why Choose 3D Galaxy</h3>
      </div>

      <div class="sticky-stack-container">
        <!-- Card 1 -->
        <div class="sticky-stack-card bg-neutral-900 text-white rounded-[2.5rem] p-10 md:p-14 border border-neutral-800 flex flex-col md:flex-row justify-between gap-8 min-h-[300px]">
          <div class="space-y-4 max-w-xl text-left">
            <span class="text-[10px] font-black tracking-widest text-orange-500 uppercase">AUTHENTICATED SYSTEMS</span>
            <h4 class="text-3xl font-black tracking-tight leading-none">Authorized National Distributor</h4>
            <p class="text-sm text-neutral-400 leading-relaxed">
              We work directly with tier-1 manufacturers to source authentic systems. Enjoy factory-certified product warranties, direct firmware overrides, and guaranteed spares support.
            </p>
          </div>
          <div class="h-20 w-20 bg-orange-500/10 border border-orange-500/25 rounded-2xl flex items-center justify-center text-orange-400 shrink-0 self-center">
        ...
        <div class="sticky-stack-card bg-neutral-900 text-white rounded-[2.5rem] p-10 md:p-14 border border-neutral-800 flex flex-col md:flex-row justify-between gap-8 min-h-[300px]">
          <div class="space-y-4 max-w-xl text-left">
            <span class="text-[10px] font-black tracking-widest text-blue-400 uppercase">CAD REVOLUTION</span>
            <h4 class="text-3xl font-black tracking-tight leading-none">Next-Gen Slicing Engine</h4>
            <p class="text-sm text-neutral-400 leading-relaxed">
              Our cloud quoting pipeline estimates parameters using direct mesh parsing. Upload your files to automatically receive custom infill, density, and filament parameters with instant pricing.
            </p>
          </div>
          <div class="h-20 w-20 bg-blue-500/10 border border-blue-500/25 rounded-2xl flex items-center justify-center text-blue-400 shrink-0 self-center">
...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section [id]="section.id" class="w-full bg-neutral-100 dark:bg-neutral-950 py-20 border-y border-neutral-200 dark:border-neutral-900 text-neutral-900 dark:text-white" appScrollReveal="fade">
...
@Component({
  ...
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section [id]="section.id" class="max-w-7xl mx-auto px-6 py-12" appScrollReveal="fade">
      <div class="text-center space-y-2 max-w-2xl mx-auto mb-12">
        <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-orange-600 font-display">Client Testimonials</h2>
        <h3 class="text-3xl md:text-4xl font-black text-neutral-900 dark:text-white tracking-tighter leading-tight font-display">Ecosystem Reviews</h3>
        <p class="text-neutral-500 dark:text-neutral-400 text-sm">See how aerospace labs, auto designers, and creators leverage our high-fidelity print ecosystem.</p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Card 1 -->
        <div appTilt [tiltMax]="5" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2.5rem] flex flex-col justify-between min-h-[250px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.03)] hover:border-orange-500/30 transition-all duration-300 text-left">
          <div class="flex items-center gap-1 text-amber-500 mb-4">
            <mat-icon class="scale-75">star</mat-icon><mat-icon class="scale-75">star</mat-icon><mat-icon class="scale-75">star</mat-icon><mat-icon class="scale-75">star</mat-icon><mat-icon class="scale-75">star</mat-icon>
          </div>
          <p class="text-sm text-neutral-500 dark:text-neutral-300 leading-relaxed italic">
            "3D Galaxy revolutionized our design verification cycle. Their Bambu AMC contracts keep our 40-printer manufacturing farm running 24/7 with zero bottlenecks."
          </p>
          <div class="flex items-center gap-4 mt-6">
            <div class="h-10 w-10 bg-neutral-100 dark:bg-neutral-800 rounded-full flex items-center justify-center font-bold text-xs">AS</div>
            <div class="space-y-0.5">
              <h4 class="text-xs font-black text-neutral-900 dark:text-white">Dr. A. Sen</h4>
              <p class="text-[10px] text-neutral-400">R&D Director, Aero-Space India</p>
            </div>
          </div>
        </div>
        ...
        <div appTilt [tiltMax]="5" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2.5rem] flex flex-col justify-between min-h-[250px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.03)] hover:border-orange-500/30 transition-all duration-300 text-left">
          ...
          <p class="text-sm text-neutral-500 dark:text-neutral-300 leading-relaxed italic">
            "Their custom quotation portal is incredibly fast. We uploaded the suspension mounts, selected PETG-CF, and got our structural batches in perfect spec."
          </p>
          <div class="flex items-center gap-4 mt-6">
            <div class="h-10 w-10 bg-neutral-100 dark:bg-neutral-800 rounded-full flex items-center justify-center font-bold text-xs">MJ</div>
            <div class="space-y-0.5">
              <h4 class="text-xs font-black text-neutral-900 dark:text-white">Meera J.</h4>
              <p class="text-[10px] text-neutral-400">Lead Product Designer, Nexa Motors</p>
            </div>
          </div>
        </div>
        ...
        <div appTilt [tiltMax]="5" class="bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-850/40 p-8 rounded-[2.5rem] flex flex-col justify-between min-h-[250px] hover:shadow-[0_20px_50px_rgba(0,0,0,0.03)] hover:border-orange-500/30 transition-all duration-300 text-left">
          ...
          <p class="text-sm text-neutral-500 dark:text-neutral-300 leading-relaxed italic">
            "Outstanding post-sales training and customer service. Our university makerspace runs completely on 3D Galaxy filaments, which yield highly consistent prints."
          </p>
          <div class="flex items-center gap-4 mt-6">
            <div class="h-10 w-10 bg-neutral-100 dark:bg-neutral-800 rounded-full flex items-center justify-center font-bold text-xs">RK</div>
            <div class="space-y-0.5">
              <h4 class="text-xs font-black text-neutral-900 dark:text-white">Prof. Rajesh K.</h4>
              <p class="text-[10px] text-neutral-400">Makerspace Admin, IIT Delhi</p>
            </div>
          </div>
        </div>
      </div>
