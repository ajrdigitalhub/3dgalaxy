import { Component, ChangeDetectionStrategy, inject, OnInit, signal, computed, effect, PLATFORM_ID, HostListener } from '@angular/core';
...
@Component({
  ...
  template: `
    ...
    <section id="products-catalog" 
      ...
      <div class="flex flex-col sm:flex-row sm:items-end justify-between border-b border-neutral-200 dark:border-neutral-800 pb-5 gap-4" appScrollReveal="fade">
        <div class="space-y-1">
          <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-orange-600 font-display">Precision Catalog</h2>
          <h3 class="text-3xl font-black text-neutral-900 dark:text-white tracking-tighter font-display">Featured Innovations</h3>
        </div>
        <div class="flex items-center gap-3 justify-center sm:justify-start">
          <button (click)="prev()" class="w-10 h-10 rounded-full border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 hover:bg-neutral-50 dark:hover:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-300 transition-colors shadow-xs cursor-pointer border-none">
            <mat-icon class="scale-90">chevron_left</mat-icon>
          </button>
          <button (click)="next()" class="w-10 h-10 rounded-full border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 hover:bg-neutral-50 dark:hover:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-300 transition-colors shadow-xs cursor-pointer border-none">
      ...
      <div class="relative w-full overflow-hidden pb-6">
        <div class="flex gap-6 select-none cursor-grab active:cursor-grabbing transition-transform"
          ...
          @for (p of featuredProducts(); track p.id + '-' + idx; let idx = $index) {
            <div [style.width]="getCardWidth()"
                 class="snap-start shrink-0">
              
              <!-- Premium Card Frame -->
              <div [appScrollReveal]="'slide-up'" 
                   [delay]="(idx % 4) * 80" 
                   appTilt 
                   [tiltMax]="8"
                ...
                <div class="absolute inset-0 pointer-events-none overflow-hidden z-20">
                  <div class="sparkle-one opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <svg class="h-6 w-6 text-amber-500 dark:text-amber-400 drop-shadow-[0_0_8px_rgba(245,158,11,0.9)]" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 0c.1 3.9 3.1 6.9 7 7-3.9.1-6.9 3.1-7 7-.1-3.9-3.1-6.9-7-7 3.9-.1 6.9-3.1 7-7z" />
                    </svg>
                  </div>
                  <div class="sparkle-two opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <svg class="h-5 w-5 text-cyan-500 dark:text-cyan-400 drop-shadow-[0_0_6px_rgba(6,182,212,0.9)]" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 0c.08 3.5 2.5 5.9 6 6-3.5.08-5.9 2.5-6 6-.08-3.5-2.5-5.9-6-6 3.5-.08 5.9-2.5 6-6z" />
                    </svg>
                  </div>
                </div>
                ...
                @if (p.isExclusive) {
                  <div class="absolute top-4 right-4 z-20 select-none pointer-events-none opacity-85 hover:opacity-100 transition-opacity">
                    <div class="relative w-14 h-14 rounded-full border border-orange-500/30 bg-orange-50/90 dark:bg-orange-955/40 flex flex-col items-center justify-center p-1 text-[7px] font-black tracking-tighter text-orange-600 dark:text-orange-400 uppercase text-center leading-none">
                      <svg class="absolute inset-0 w-full h-full -rotate-12" viewBox="0 0 100 100">
                        <circle cx="50" cy="50" r="46" fill="none" stroke="currentColor" stroke-width="1.2" stroke-dasharray="2,2" />
                      </svg>
                      <span class="text-[5px] scale-90 tracking-widest font-black">EXCLUSIVE</span>
                      <span class="font-extrabold text-[8px] my-0.5 text-nowrap tracking-wide">{{ p.brand }}</span>
                      <span class="text-[5px] scale-90 font-black">DISTRIBUTOR</span>
                    </div>
                  </div>
                }

                <!-- Product Image container -->
                <div class="relative aspect-square bg-neutral-50/50 dark:bg-neutral-950/20 rounded-2xl border border-neutral-100/80 dark:border-neutral-800/20 flex items-center justify-center overflow-hidden p-3 mb-4 group-hover:scale-[0.98] transition-transform duration-500 product-card-image-container">
                  <div class="shimmer-sweep absolute inset-0 pointer-events-none z-10"></div>
                  
                  <!-- Main Image -->
                  <img [src]="p.images && p.images[0]?.url || p.images && p.images[0] || 'https://via.placeholder.com/400x400?text=No+Image'"
                  ...
                  @if (p.images && p.images.length > 1) {
                    <img [src]="p.images[1]?.url || p.images[1]"
                      class="absolute inset-0 w-[85%] h-[85%] m-auto object-contain secondary-image z-2"
                      alt="{{p.name}} Alternate" loading="lazy">
                  }

                  <!-- Offer/Discount Badge -->
                  @if (p.mrp && p.mrp > p.activePrice) {
                    <span class="absolute top-3 left-3 px-2.5 py-1 bg-orange-600 text-white text-[9px] font-black rounded-lg uppercase tracking-wider z-10 shadow-md">
                      {{ getDiscountPercent(p) }}% OFF
                    </span>
                  }

                  <!-- Wishlist Button -->
                  <button (click)="$event.stopPropagation(); toggleWishlist(p.id)"
                    ...
                    <mat-icon class="scale-90">{{ wishlistIds().has(p.id) ? 'favorite' : 'favorite_border' }}</mat-icon>
                  </button>
                  
                  <!-- Stock Status Badge -->
                  <span class="absolute bottom-3 left-3 px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-wider z-10"
                        [ngClass]="p.stock > 0 ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/15' : 'bg-rose-500/10 text-rose-500 border border-rose-500/15'">
                ...
                <div class="flex-1 flex flex-col space-y-2 text-left">
                  <span class="text-[9px] font-black uppercase tracking-[0.2em] text-orange-500">{{ p.brand }}</span>
                  ...
                  <!-- Short Description -->
                  <p class="text-xs text-neutral-500 dark:text-neutral-400 font-medium line-clamp-2 min-h-[32px] leading-relaxed">
                    {{ p.shortDescription || p.description || 'Premium quality 3D fabrication component.' }}
                  </p>

                  <div class="flex items-center justify-between mt-auto pt-2">
                    <div class="flex flex-col">
                      <span class="text-base font-black text-neutral-900 dark:text-white">{{ p.activePrice | number:'1.0-0' }} {{ currency() }}</span>
                      @if (p.mrp && p.mrp > p.activePrice) {
                        <span class="text-[10px] text-neutral-400 line-through font-normal">{{ p.mrp | number:'1.0-0' }} {{ currency() }}</span>
                      }
                    </div>
                    <div class="flex items-center gap-1 bg-amber-500/10 text-amber-500 px-2 py-0.5 rounded-md">
                      <mat-icon class="scale-75 -mr-1">star</mat-icon>
                      <span class="text-[10px] font-black">{{ p.calculatedRating }}</span>
                    </div>
                  </div>

                  <!-- CTAs -->
                  <div class="grid grid-cols-2 gap-2 pt-2">
                    <button (click)="$event.stopPropagation(); buyNow(p)"
                            [disabled]="p.stock <= 0"
...
export class HomeFeaturedProductsComponent implements OnInit {
  ...
  originalProductsLength = computed(() => {
    ...
    return list.length > 0 ? list.length : Math.min(this.ds.products().length, 8);
  });
  ...
  featuredProducts = computed(() => {
    ...
    const mapped = result.map(p => {
      const price = isDealer ? (p.dealerPrice || p.dealer_price || p.salePrice || p.sale_price) : (p.salePrice || p.sale_price || p.basePrice || p.mrp);
