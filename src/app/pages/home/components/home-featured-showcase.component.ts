import { Component, ChangeDetectionStrategy, inject, signal, computed, effect, ElementRef, viewChild, PLATFORM_ID, HostListener, OnInit } from '@angular/core';
...
@Component({
  ...
  template: `
    ...
    <section #showcaseContainer id="featured-showcase-section"
      ...
      <div class="relative w-full min-h-[500px] flex items-center justify-center"
        ...
        @if (featuredProducts().length === 0) {
          ...
          <div class="w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div class="lg:col-span-6 flex justify-center">
              <div class="w-[80%] aspect-square bg-neutral-100 dark:bg-neutral-800 rounded-3xl animate-pulse"></div>
            </div>
            <div class="lg:col-span-6 space-y-6">
              <div class="h-6 w-32 bg-neutral-100 dark:bg-neutral-800 rounded-full animate-pulse"></div>
              <div class="h-16 w-3/4 bg-neutral-100 dark:bg-neutral-800 rounded-2xl animate-pulse"></div>
              <div class="h-24 w-full bg-neutral-100 dark:bg-neutral-800 rounded-2xl animate-pulse"></div>
              <div class="h-12 w-40 bg-neutral-100 dark:bg-neutral-800 rounded-xl animate-pulse"></div>
            </div>
          </div>
        } @else {
          <!-- Slides Loop -->
          @for (product of featuredProducts(); track product.id || $index; let idx = $index) {
            <div [class]="currentSlide() === idx ? 'opacity-100 block relative z-10 w-full' : 'opacity-0 hidden absolute inset-0 pointer-events-none z-0'"
              ...
              <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center w-full">
                ...
                <div [class]="(idx % 2 === 0 ? 'lg:col-span-6 lg:order-1' : 'lg:col-span-6 lg:order-2') + ' order-1 flex justify-center items-center overflow-hidden w-full'"
                     appTilt [tiltMax]="5">
                  <div class="relative max-h-[450px] lg:max-h-[550px] w-full flex justify-center items-center p-4">
                    <img [src]="product.images?.[0] || 'https://picsum.photos/seed/' + product.slug + '/800/800'"
                         [alt]="product.name"
                         class="max-h-[380px] lg:max-h-[500px] w-auto object-contain hover:scale-105 transition-transform duration-700 ease-out drop-shadow-2xl animate-float-slow gpu-accelerated"
                         (error)="onImageError($event)">
                  </div>
                </div>

                <!-- CONTENT COLUMN (order-2 stacks second on mobile/tablet) -->
                <div [class]="(idx % 2 === 0 ? 'lg:col-span-6 lg:order-2' : 'lg:col-span-6 lg:order-1') + ' order-2 space-y-6 text-left flex flex-col justify-center w-full px-2 lg:px-8'">
                  <span class="showcase-anim text-xs font-black uppercase tracking-[0.3em] text-[#d65108] font-display block select-none">
                    {{ product.brand || '3D GALAXY' }}
                  </span>
                  <h3 class="showcase-anim text-3xl md:text-4xl lg:text-5xl font-black text-neutral-900 dark:text-white tracking-tighter leading-tight font-display select-none">
                    {{ product.name }}
                  </h3>
                  <p class="showcase-anim text-sm md:text-base text-neutral-500 dark:text-neutral-400 font-medium leading-relaxed max-w-xl select-none line-clamp-4">
                    {{ product.description || product.shortDescription }}
                  </p>
                  
                  <div class="showcase-anim flex items-baseline gap-3 select-none">
                    @if (product.sale_price || product.salePrice; as salePrice) {
                      <span class="text-3xl font-black text-neutral-900 dark:text-white font-mono">
                        ₹{{ salePrice | number:'1.0-0' }}
                      </span>
                      @if (product.mrp && product.mrp > salePrice) {
                        <span class="text-lg text-neutral-400 line-through font-mono">
        ...
        <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
          @for (dot of featuredProducts(); track $index) {
            <button (click)="setSlide($index)"
                    [class]="currentSlide() === $index ? 'w-8 bg-blue-600' : 'w-2 bg-neutral-300 dark:bg-neutral-700 hover:bg-neutral-400'"
                    class="h-2 rounded-full transition-all duration-300 border-none cursor-pointer p-0"
                    [attr.aria-label]="'Go to slide ' + ($index + 1)"></button>
          }
        </div>
      }
    </section>
