      ...
      try {
        ...
        const tryNext = () => {
          ...
          if (busboyFinished && (writeFinished || !fileHandled)) {
            console.log(
              "[UPLOAD] All done. req.file:",
              req.file ? req.file.filename : "NOT SET",
            );
            next();
          }
        };
        ...
        bb.on("file", (name, fileStream, info) => {
          ...
          if (!mimeTypeValid || !extNameValid) {
            ...
            return next(
              new Error(
                `Invalid file type. Allowed extensions: ${allowedExtensions}`,
              ),
            );
          }
          ...
          writeStream.on("finish", () => {
            ...
            console.log(
              "[UPLOAD] writeStream finished. file saved:",
              filepath,
              "size:",
              fileSize,
            );
        ...
        bb.on("close", () => {
          console.log(
            "[UPLOAD] busboy close. fileHandled:",
            fileHandled,
            "writeFinished:",
            writeFinished,
          );
        ...
        req.on("error", (err) => {
          if (!errorOccurred) {
            errorOccurred = true;
            console.log("[UPLOAD] request stream error:", err);
            next(err);
          }
        });
