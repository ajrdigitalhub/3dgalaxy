...
export const registerDevice = async (req: Request, res: Response) => {
  ...
  if (!fcmToken) {
    return res.status(400).json({ status: 'error', success: false, message: 'FCM Token is required.', error: 'FCM Token is required.' });
  }
  ...
  try {
    ...
    if (existing) {
      ...
      const updated = await prisma.notificationDevice.update({
        ...
        data: {
          ...
          notificationEnabled: notificationEnabled !== undefined ? notificationEnabled : existing.notificationEnabled,
          lastActive: new Date()
        }
      });
      return res.status(200).json({ status: 'success', success: true, message: 'Device token updated.', data: updated });
    }
    ...
    return res.status(201).json({ status: 'success', success: true, message: 'Device registered successfully.', data: created });
  } catch (error: any) {
    console.error('Failed to register device:', error);
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to register device.', error: 'Failed to register device.', details: error.message });
  }
};
...
export const updateToken = async (req: Request, res: Response) => {
  ...
  if (!oldToken || !newToken) {
    return res.status(400).json({ status: 'error', success: false, message: 'Both oldToken and newToken are required.', error: 'Both oldToken and newToken are required.' });
  }
  ...
  try {
    ...
    if (!existing) {
      ...
      return res.status(201).json({ status: 'success', success: true, message: 'New token registered.', data: created });
    }
    ...
    return res.status(200).json({ status: 'success', success: true, message: 'Token updated successfully.', data: updated });
  } catch (error: any) {
    console.error('Failed to update token:', error);
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to update token.', error: 'Failed to update token.', details: error.message });
  }
};
...
export const subscribeToTopic = async (req: Request, res: Response) => {
  ...
  if (!fcmToken || !topic) {
    return res.status(400).json({ status: 'error', success: false, message: 'fcmToken and topic are required.', error: 'fcmToken and topic are required.' });
  }

  const messaging = getMessaging();
  if (!messaging) {
    return res.status(500).json({ status: 'error', success: false, message: 'FCM push notifications not configured on server.', error: 'FCM push notifications not configured on server.' });
  }

  try {
    const response = await messaging.subscribeToTopic([fcmToken], topic);
    return res.status(200).json({ status: 'success', success: true, message: 'success', data: response });
  } catch (error: any) {
    console.error(`Failed to subscribe to topic ${topic}:`, error);
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to subscribe to topic.', error: 'Failed to subscribe to topic.', details: error.message });
  }
};
...
export const unsubscribeFromTopic = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to unsubscribe from topic.', error: 'Failed to unsubscribe from topic.', details: error.message });
  }
};
...
export const getNotifications = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (userId) {
      ...
      return res.status(400).json({ status: 'error', success: false, message: 'Either userId or guestId is required.', error: 'Either userId or guestId is required.' });
    }
    ...
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to fetch notifications.', error: 'Failed to fetch notifications.', details: error.message });
  }
};
...
export const markRead = async (req: Request, res: Response) => {
  ...
  try {
    if (ids && Array.isArray(ids)) {
      ...
      return res.status(400).json({ status: 'error', success: false, message: 'id or ids array is required.', error: 'id or ids array is required.' });
    }

    return res.status(200).json({ status: 'success', success: true, message: 'Notifications marked as read.', data: null });
  } catch (error: any) {
    console.error('Failed to mark notifications as read:', error);
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to mark read.', error: 'Failed to mark read.', details: error.message });
  }
};
...
export const deleteNotification = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json({ status: 'success', success: true, message: 'Notification deleted successfully.', data: null });
  } catch (error: any) {
    console.error('Failed to delete notification:', error);
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to delete notification.', error: 'Failed to delete notification.', details: error.message });
  }
};
...
export const sendPushNotificationInternal = async (params: {
  ...
  const { targetType, targetValue, title, body, image, actionUrl, type } = params;
  ...
  if (targetType === 'all') {
    ...
    if (!targetValue) throw new Error('targetValue required for device target.');
    tokens = [targetValue];
  ...
  if (isTopic) {
    ...
    if (multicastResponse.failureCount > 0) {
      finalStatus = multicastResponse.failureCount === tokens.length ? 'FAILED' : 'PARTIAL';
      ...
      multicastResponse.responses.forEach((resp, idx) => {
        if (!resp.success) {
          ...
          if (errCode === 'messaging/invalid-registration-token' || errCode === 'messaging/registration-token-not-registered') {
            prisma.notificationDevice.delete({ where: { fcmToken: tokens[idx] } }).catch(() => {});
          }
        }
      });
    }
    ...
    const uniqueRecipientUsers = Array.from(new Set(devices.map(d => d.userId).filter(id => id !== null))) as string[];
    const guestIds = Array.from(new Set(devices.filter(d => d.userId === null && d.guestId !== null).map(d => d.guestId))) as string[];

    const inboxData: any[] = [];
    uniqueRecipientUsers.forEach(uid => {
      inboxData.push({ userId: uid, guestId: null, title, body, image, actionUrl });
    });
    guestIds.forEach(gid => {
      inboxData.push({ userId: null, guestId: gid, title, body, image, actionUrl });
    });
...
export const adminSendNotification = async (req: Request, res: Response) => {
  ...
  if (!title || !body) {
    return res.status(400).json({ status: 'error', success: false, message: 'Title and body are required.', error: 'Title and body are required.' });
  }
  ...
  try {
    ...
    return res.status(200).json({ status: 'success', success: true, message: 'Notification sent successfully.', data: responsePayload });
  } catch (error: any) {
    console.error('Failed to send notification:', error);
    return res.status(500).json({ status: 'error', success: false, message: error.message || 'Failed to send notification.', error: 'Failed to send notification.', details: error.message });
  }
};
...
export const adminSendUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (devices.length === 0) {
      ...
      return res.status(200).json({ status: 'success', success: true, message: 'User has no active devices. Saved to inbox.', data: null });
    }
...
export const adminGetAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    totalLogs.forEach(log => {
      ...
      else if (log.status === 'PARTIAL') delivered++; // counts partially delivered too
      else failed++;
    });
