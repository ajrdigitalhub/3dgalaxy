...
export class TrackingService {
  /**
   * Retrieves the list of courier partners merged with custom admin configurations.
   */
  static getCourierList(customCourierSettings?: any[]): CourierPartnerConfig[] {
    if (!customCourierSettings || !Array.isArray(customCourierSettings) || customCourierSettings.length === 0) {
    ...
    customCourierSettings.forEach(c => {
  ...
  /**
   * Automatically generates tracking URL from Courier Partner and Tracking Number.
   */
  ...
  /**
   * Automatically calculates Estimated Delivery date range based on product shipping rules and shipment date.
   */
  static calculateEstimatedDelivery(
    ...
    if (items && Array.isArray(items) && items.length > 0) {
      for (const item of items) {
        ...
        const decoded = decodeDays(encodedDays);
