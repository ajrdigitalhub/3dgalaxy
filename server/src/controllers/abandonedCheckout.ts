...
async function getSettings() {
  ...
  return data.abandonedCheckoutSettings || {
    ...
    autoRecovery: true,
    recoverySchedule: [30, 120, 1440], // in minutes
    sendEmail: true,
    sendWhatsapp: true,
...
export const logCartActivity = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Skip if config excludes logged or guest users
    ...
    // Find existing active checkout for this session
    ...
    // Log the activity to the chronological logs
...
export const logCheckoutHeartbeat = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Create activity logs if checkout step has progressed
...
export const recoverCartByToken = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Mark as active recovery in timeline
    await prisma.checkoutActivityLog.create({
      data: {
        abandonedCheckoutId: checkout.id,
    ...
    // Update status to Converted if they complete but here we just return the payload
...
// 5. GET /api/admin/abandoned-checkouts
...
// 6. GET /api/admin/abandoned-checkouts/:id
...
// Helper for simulated notification dispatches
async function dispatchSimulatedReminder(checkout: any, channel: 'EMAIL' | 'WHATSAPP', reminderStep: number) {
  ...
  if (channel === 'WHATSAPP') {
    try {
      // Import trigger function dynamically to avoid load-order circular imports
...
// 7. POST /api/admin/abandoned-checkouts/resend
export const manualResendReminder = async (req: Request, res: Response) => {
  const { id, channel } = req.body; // channel: 'EMAIL' | 'WHATSAPP'
  ...
  try {
    ...
    await dispatchSimulatedReminder(checkout, channel || 'EMAIL', 99); // 99 indicates manual push
...
// 8. POST /api/admin/abandoned-checkouts/recover
...
// 9. GET /api/admin/abandoned-checkouts/analytics
export const getAdminAbandonedCheckoutsAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    // Browser / Device / Payment Method trends
...
// Background automatic cron scanner
export const startAbandonedCheckoutRecoveryWorker = () => {
  setInterval(async () => {
    try {
      const settings = await getSettings();
      if (!settings.enabled || !settings.autoRecovery) return;

      const timeoutMs = (settings.timeoutMinutes || 30) * 60 * 1000;
      const now = new Date();

      // Find checkouts with status 'Active', no orders placed, and lastActivity past timeout
      const checkouts = await prisma.abandonedCheckout.findMany({
        where: {
          recoveryStatus: 'Active',
          checkoutStep: { not: 'COMPLETED' },
          lastActivity: { lt: new Date(now.getTime() - timeoutMs) }
        },
        include: {
          notifications: true
        }
      ...
      for (const checkout of checkouts) {
        const minutesElapsed = (now.getTime() - new Date(checkout.lastActivity).getTime()) / 60000;

        // Schedules: e.g. [30, 120, 1440] in minutes
        const schedule = settings.recoverySchedule || [30, 120, 1440];

        for (let i = 0; i < schedule.length; i++) {
          const targetMin = schedule[i];
          const reminderStep = i + 1;

          // Check if this step was already sent
          const alreadySent = checkout.notifications.some(n => n.reminderStep === reminderStep);
          if (alreadySent) continue;

          // If elapsed time is greater than schedule target (with a buffer)
          if (minutesElapsed >= targetMin && minutesElapsed < targetMin + 120) {
            // Trigger dispatches
            if (settings.sendEmail && checkout.email) {
              await dispatchSimulatedReminder(checkout, 'EMAIL', reminderStep);
            }
            if (settings.sendWhatsapp && checkout.mobile) {
              await dispatchSimulatedReminder(checkout, 'WHATSAPP', reminderStep);
            }
            break; // trigger only one reminder per tick
          }
        }
      }
    } catch (e) {
      console.error('Error in background Abandoned Checkout worker:', e);
    }
