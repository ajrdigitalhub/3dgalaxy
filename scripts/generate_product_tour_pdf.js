...
const HTML_FILE = path.resolve(__dirname, '..', 'docs', 'assets', 'product-configuration-tour', 'product_tour_print.html');
const DOCS_PDF = path.resolve(__dirname, '..', 'docs', '3D_Galaxy_Product_Configuration_Visual_Tour_Guide.pdf');
const ROOT_PDF = path.resolve(__dirname, '..', '3D_Galaxy_Product_Configuration_Visual_Tour_Guide.pdf');
...
async function run() {
  ...
  const headerHtml = `
    ...
    <div style="width: 100%; font-size: 7.2pt; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; font-weight: 700; display: flex; justify-content: space-between; align-items: center; padding: 7mm 18mm 4px 18mm; border-bottom: 1.5px solid #cbd5e1; box-sizing: border-box;">
      ...
      <span style="color: #0284c7; text-transform: uppercase; letter-spacing: 0.8px; font-weight: 800;">PRODUCT CONFIGURATION VISUAL TOUR GUIDE</span>
    </div>
  `;
  ...
  const footerHtml = `
    ...
    <div style="width: 100%; font-size: 7pt; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; display: flex; justify-content: space-between; align-items: center; padding: 4px 18mm 7mm 18mm; border-top: 1px solid #cbd5e1; box-sizing: border-box; color: #64748b;">
      <span>DOC-3DG-PRD-TOUR-2026-V1.0 • 3D Galaxy Product Configuration Tour • AJR Digital Hub</span>
  ...
  console.log('=====================================================');
  console.log('SUCCESS! Visual Tour Guide PDF generated successfully:');
...
run().catch(err => {
  console.error('Visual Tour Guide PDF Generation Failed:', err);
  process.exit(1);
});
