...
const TARGET_PDFS = [
  path.resolve(__dirname, '..', 'docs', '3D_Galaxy_Product_Configuration_Guide.pdf'),
  path.resolve(__dirname, '..', '3D_Galaxy_Product_Configuration_Guide.pdf'),
  path.resolve(__dirname, '..', 'docs', '3D_Galaxy_Product_Configuration_Master_Guide.pdf'),
  path.resolve(__dirname, '..', '3D_Galaxy_Product_Configuration_Master_Guide.pdf'),
  path.resolve(__dirname, '..', 'docs', '3D_Galaxy_Product_Configuration_Visual_Tour_Guide.pdf'),
  path.resolve(__dirname, '..', '3D_Galaxy_Product_Configuration_Visual_Tour_Guide.pdf')
];
...
async function run() {
  ...
  // 1. Render Cover Page (Page 1) without running header/footer and zero margin bleed
  ...
  const headerHtml = `
    ...
    <div style="width: 100%; font-size: 7.2pt; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; font-weight: 700; display: flex; justify-content: space-between; align-items: center; padding: 7mm 18mm 4px 18mm; border-bottom: 1.5px solid #cbd5e1; box-sizing: border-box;">
      ...
      <span style="color: #0284c7; text-transform: uppercase; letter-spacing: 0.8px; font-weight: 800;">PRODUCT CONFIGURATION ADMINISTRATION GUIDE</span>
    </div>
  `;
  ...
  const footerHtml = `
    ...
    <div style="width: 100%; font-size: 7pt; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; display: flex; justify-content: space-between; align-items: center; padding: 4px 18mm 7mm 18mm; border-top: 1px solid #cbd5e1; box-sizing: border-box; color: #64748b;">
      <span>DOC-3DG-PRD-ADMIN-2026-V2.0 • 3D Galaxy Product Configuration Guide • AJR Digital Hub</span>
  ...
  for (const targetPath of TARGET_PDFS) {
    fs.writeFileSync(targetPath, finalPdfBytes);
    console.log('  ✓ ' + targetPath);
  }

  const stats = fs.statSync(TARGET_PDFS[0]);
  console.log('=====================================================');
  console.log('SUCCESS! Clean Product Configuration Guide PDF generated:');
...
run().catch(err => {
  console.error('Clean Guide PDF Generation Failed:', err);
  process.exit(1);
});
