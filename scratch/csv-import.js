...
var csv_import_exports = {};
__export(csv_import_exports, {
  buildShopifyImportGroups: () => buildShopifyImportGroups,
  parseCsv: () => parseCsv
});
module.exports = __toCommonJS(csv_import_exports);
var import_sync = require("csv-parse/sync");
var HEADER_NORMALIZATION = {
  title: "title",
  handle: "handle",
  body_html: "body_html",
  vendor: "vendor",
...
var parseCsv = (csvText) => {
  try {
    const records = (0, import_sync.parse)(csvText, {
      columns: (headers2) => headers2.map(normalizeField),
...
var splitCsvArray = (value) => {
  if (!value) return [];
  return value.split(/[\n\r,|;]+/).map((item) => item.trim()).filter(Boolean);
};
var parseSpecificationsField = (value) => {
  if (!value) return [];
  ...
  if (parsedJson && typeof parsedJson === "object") {
    return Object.entries(parsedJson).map(([name, value2]) => ({
      name: String(name),
      value: String(value2)
    }));
  }
...
var parseIncludedItemsField = (value) => {
  if (!value) return [];
  return splitCsvArray(value);
};
var buildVariantFromRow = (row, firstRow) => {
  const optionValues = {};
  [1, 2, 3].forEach((index) => {
    const optionName = getValue(row, `option${index}_name`) || getValue(row, `option${index}`) || (firstRow ? getValue(firstRow, `option${index}_name`) || getValue(firstRow, `option${index}`) : "");
    const optionValue = getValue(row, `option${index}_value`) || getValue(row, `option${index}`);
  ...
  const rawCompare = getValue(row, "variant_compare_at_price") || getValue(row, "variant_price_compare_at") || getValue(row, "compare_price");
  const rawStock = getValue(row, "variant_inventory_qty") || getValue(row, "inventory_quantity") || getValue(row, "stock");
  const rawWeight = getValue(row, "variant_grams") || getValue(row, "grams") || getValue(row, "weight");
  ...
  const name = variantTitle || (optionValuesList.length > 0 ? optionValuesList.join(" / ") : "") || getValue(row, "title") || getValue(row, "variant_sku") || "Default Title";
  return {
    name,
...
var buildShopifyImportGroups = (parsed, mapping = {}) => {
  const groups = {};
  const skuRegistry = /* @__PURE__ */ new Set();
  const duplicateSkuRegistry = /* @__PURE__ */ new Set();
  const resolveKey = (row) => {
    const handle = getValue(row, "handle");
    const title = getValue(row, "title");
    const slug = getValue(row, "slug");
    const fallback = title || slug || getValue(row, "variant_sku") || getValue(row, "sku") || "";
    return handle || slug || fallback || `row-${Math.random().toString(36).slice(2, 8)}`;
  };
  const resolveText = (row, keys) => {
  ...
  const buildAttributeName = (key) => key.replace(/product_metafields_shopify_/g, "").replace(/product_metafields_mm_google_shopping_/g, "").replace(/google_shopping_/g, "").replace(/product_metafields_/g, "").replace(/_/g, " ").replace(/\b(\w)/g, (match) => match.toUpperCase()).trim();
  const extractAdditionalAttributes = (row) => {
  ...
  for (const row of parsed.rows) {
    ...
    const published = toBoolean(getValue(row, "published")) || toBoolean(getValue(row, "status"));
    ...
    if (isNewProduct) {
      groups[key] = {
        key,
        name: title || key,
        slug: getValue(row, "handle") || getValue(row, "slug") || title.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
        vendor,
        productType,
        productCategory,
        tags,
    ...
    const hasVariantDetails = getValue(row, "variant_sku") !== "" || getValue(row, "sku") !== "" || getValue(row, "option1_value") !== "" || getValue(row, "option2_value") !== "" || getValue(row, "option3_value") !== "" || getValue(row, "variant_price") !== "" && getValue(row, "variant_price") !== "0" || getValue(row, "price") !== "" && getValue(row, "price") !== "0";
    if (isNewProduct || hasVariantDetails) {
      const variant = buildVariantFromRow(row, product.rawRows[0]);
      const duplicateCheck = variant.sku ? variant.sku : void 0;
      if (duplicateCheck) {
  ...
  const result = Object.values(groups).map((product) => {
    const uniqueVariantKeys = /* @__PURE__ */ new Set();
    const variants = product.variants.filter((variant, index) => {
      const variantKey = variant.sku || variant.name || JSON.stringify(variant.optionValues || {}) || `${product.key}-${index}`;
    ...
    if (!product.name || variants.length === 0) {
      if (!normalizedErrors.includes("Missing required fields: title or variants")) {
    ...
    return {
      ...
      options: Array.from(
        new Set(
          variants.flatMap(
            (variant) => Object.keys(variant.optionValues || {})
          )
        )
      ).map((optionName) => ({
        name: optionName,
        values: uniqueArray(
          variants.map((variant) => variant.optionValues?.[optionName] || "").filter(Boolean)
        )
      }))
    };
  }).map((product) => {
    const variantsMissingImages = product.variants.filter(
      (variant) => !(variant.variantImages || variant.images || []).length
    );
    if (variantsMissingImages.length > 0 && product.images.length >= product.variants.length) {
      product.variants = product.variants.map((variant, index) => {
        ...
        const inferred = product.images[index] ? [product.images[index]] : [];
        return {
          ...variant,
